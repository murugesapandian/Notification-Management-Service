...
| Database (dev/test) | H2 (in-memory, `MODE=PostgreSQL`) |
...
- **Nothing else required for local/dev**: H2 runs in-memory, no external services needed.
- **PostgreSQL 14+** — only needed if you run the backend with the `postgres` Spring profile
  instead of the default in-memory setup.
...
The service starts on **http://localhost:8080**, applies Flyway migrations to an in-memory H2
database automatically, and starts the scheduled delivery/escalation/cleanup workers.

Useful endpoints once running:

- Swagger UI: http://localhost:8080/swagger-ui.html
- OpenAPI spec: http://localhost:8080/v3/api-docs
- H2 console (dev only): http://localhost:8080/h2-console — JDBC URL `jdbc:h2:mem:nms`, user `sa`, no password
